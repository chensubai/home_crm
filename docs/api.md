...
普通提醒和周期提醒在目标时间前 30 分钟通知，`item_expiry` 在保质期前 2 天通知。服务端不发送 APNs 推送。
物品设置 `expires_at` 后，服务端会自动创建家庭共享的 `item_expiry` 提醒；通知正文为「物品名称还有两天过期」。清除保质期或删除物品时，对应提醒会被软删除。家庭成员各自同步后，会在自己的设备上注册本地通知。
