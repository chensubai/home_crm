...
struct APIClient {
    ...
    func createSpace(familyId: Int, name: String, nfcUid: String?) async throws -> SpaceDTO {
        try await request("spaces", method: "POST", body: SpaceCreateRequest(familyId: familyId, name: name, nfcUid: nfcUid))
    }
    ...
    func createItem(_ payload: [String: EncodableValue]) async throws -> ItemDTO {
        try await request("items", method: "POST", body: payload)
    }
